...
def calc_active(field, x, y, z):
    neighbours = 0
    for n_z in range(z - 1, z + 2):
        for n_y in range(y - 1, y + 2):
            for n_x in range(x - 1, x + 2):
                if (n_x, n_y, n_z) == (x, y, z):
                    continue
                if get_field(field, n_x, n_y, n_z) == '#':
                    neighbours += 1
    return neighbours
...
def set_field(field, x, y, z, state):
    if x not in field:
        field[x] = {}
    if y not in field[x]:
        field[x][y] = {}
...
def get_mins_and_maxs_4d(field):
    min_x, max_x = min(field.keys()), max(field.keys())
    min_y, max_y = min(c for x in field.keys() for c in field[x].keys()), max(c for x in field.keys() for c in field[x].keys())
    min_z, max_z = min(c for x in field.keys() for y in field[x].keys() for c in field[x][y].keys()), max(c for x in field.keys() for y in field[x].keys() for c in field[x][y].keys())
...
def get_active(field):
    min_x, max_x, min_y, max_y, min_z, max_z = get_mins_and_maxs(field)
    result = 0
    for z in range(min_z, max_z + 1):
        for y in range(min_y, max_y + 1):
            for x in range(min_x, max_x + 1):
                if get_field(field, x, y, z) == '#':
                    result += 1
    return result

def simulate_4d(field, cycles=6):
                ...
                for y in range(min_y - 1, max_y + 2):
                    for x in range(min_x - 1, max_x + 2):
                        ...
                        if get_field_4d(field, x, y, z, w) == '.' and n == 3:
                            ...
                            set_field_4d(new_field, x, y, z, w, get_field_4d(field, x, y, z, w))
                        
        field = new_field

    return get_active_4d(field)

def simulate(field, cycles=6):
    for c in range(cycles):
        min_x, max_x, min_y, max_y, min_z, max_z = get_mins_and_maxs(field)
        new_field = {}
        for z in range(min_z - 1, max_z + 2):
            for y in range(min_y - 1, max_y + 2):
                for x in range(min_x - 1, max_x + 2):
                    n = calc_active(field, x, y, z)
                    if get_field(field, x, y, z) == '.' and n == 3:
                        set_field(new_field, x, y, z, '#')
                    elif get_field(field, x, y, z) == '#' and n not in [2, 3]:
                        set_field(new_field, x, y, z, '.')
                    else:
                        set_field(new_field, x, y, z, get_field(field, x, y ,z))
                    
        field = new_field

    return get_active(field)
...
def read_field(line_gen):
    field = {}
    y = 0
    for line in line_gen:
        for idx, char in enumerate(line):
            set_field(field, idx, y, 0, char)
        y += 1
    return field

def part_one(line_gen):
    return simulate(read_field(line_gen))

def part_two(line_gen):
    return simulate_4d(read_field_4d(line_gen))
